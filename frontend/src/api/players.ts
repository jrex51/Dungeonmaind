...
export type AbilitiesPatch = Partial<{
  str: number
  dex: number
  con: number
  int_: number
...
export async function updateAbilities(playerId: string, patch: AbilitiesPatch): Promise<PlayerOut> {
  const url = new URL(`/players/${playerId}`, SERVER_CONFIG.BASE_URL).toString()
  const res = await fetch(url, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      'X-Player-Id': playerId, // <— Self-Permission
    },
